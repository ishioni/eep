...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		{name: "null theme", content: `{"theme":null}`, wantErr: `field "theme" must be a string`},
		{name: "invalid theme type", content: `{"theme":false}`, wantErr: `field "theme" must be a string`},
		{name: "empty theme", content: `{"theme":""}`, wantErr: "must not be empty"},
		{name: "invalid show details type", content: `{"showDetails":"true"}`, wantErr: `field "showDetails" must be a boolean`},
		{name: "null show details", content: `{"showDetails":null}`, wantErr: `field "showDetails" must be a boolean`},
		{name: "null locale", content: `{"locale":null}`, wantErr: `field "locale" must be a string`},
		{name: "invalid locale type", content: `{"locale":false}`, wantErr: `field "locale" must be a string`},
		{name: "empty locale", content: `{"locale":""}`, wantErr: "must not be empty"},
		{name: "null log level", content: `{"logLevel":null}`, wantErr: `field "logLevel" must be a string`},
		{name: "invalid log level type", content: `{"logLevel":false}`, wantErr: `field "logLevel" must be a string`},
		{name: "empty log level", content: `{"logLevel":""}`, wantErr: "must not be empty"},
