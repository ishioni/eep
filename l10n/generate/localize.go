...
import (
	...
	"github.com/ishioni/eep/l10n/generate/jsmin"
)
...
func main() {
	var localesFile, jsOutFile, jsMinOutFile, goOutFile, playgroundFile string
	...
	flag.StringVar(&jsMinOutFile, "out-min", "localize.min.js", "output minified JavaScript file path")
	flag.StringVar(&goOutFile, "out-go", "./supported_locales.go", "output supported locales Go file path")
	...
	exitIfErr(writeSupportedLocalesFile(locales, goOutFile), "writing supported locales file")
...
func writeSupportedLocalesFile(locales localesData, filePath string) error {
	seen := make(map[string]struct{})
	for _, translations := range locales {
		for langCode := range translations {
			seen[strings.TrimSpace(strings.ToLower(langCode))] = struct{}{}
		}
	}

	codes := slices.Collect(maps.Keys(seen))
	sort.Strings(codes)

	var source bytes.Buffer
	source.WriteString("// Code generated by go generate. DO NOT EDIT.\n\npackage l10n\n\n")
	source.WriteString("var supportedLocaleCodes = [...]string{ //nolint:gochecknoglobals\n")
	...
	return os.WriteFile(filePath, formatted, 0o644) //nolint:gosec,mnd
}
